...
class RewardLearningLocal(Algorithm):
    def __init__(
        self,
        lambda_=1,
        sigma=1,
        ...
        if self.person == "Target":
            ...
            # if self.use_prior:
                # self.rwd_theta_prior = np.array([1, -1, -1, 1, -0.5, -0.5])
            self.rwd_prior_var = np.eye(self.rwd_dim) / self.rwd_lambda
            self.rwd_variance = np.eye(self.rwd_dim) / 0.1
            self.rwd_variance_inv = np.eye(self.rwd_dim) * 0.00001
        else:
            # if self.use_prior:
                # self.rwd_theta_prior = np.array([1, 1, -1, -1])
                # self.rwd_theta_prior = np.array([1, -1])
            self.rwd_prior_var = np.eye(self.rwd_dim) / self.rwd_lambda
            self.rwd_variance = np.eye(self.rwd_dim) / 0.1
            self.rwd_variance_inv = np.eye(self.rwd_dim) * 0.00001
    ...
    def getRewardFeature(self, state, action):
        ...
        if self.person == "Target":
            ...
            short_state = np.append(state[[0, 3, 4, 5]], action) # care-giver uses its stress and current burden
            feature = short_state
            # feature = np.outer(short_state, oneHot, out=None).T.reshape(-1)
        else:
            # short_state = state[[0, 5, 2]]
            feature = np.append(state[[0, 5, 2]], [action, action * state[5], action * state[2]])
            # feature = np.outer(short_state, oneHot, out=None).T.reshape(-1)
        return feature
    ...
    def reward_learning(self):
        ...
        if self.person == "Care":
            ...
            tmp_X = self.rwd_X[:-7]
            # rolling_window = 14
            # if len(tmp_y) >= rolling_window:
            #     tmp_y = np.convolve(tmp_y, np.ones(rolling_window)/rolling_window, mode='same')
        ...
        if self.save_param and self.rwd_update_count % 1 == 0 and self.person in ["Care", "Game"]:
            ...
            if self.name is None:
                self.name = ""
            with open(f"{save_dir}_theta_rwd_{self.person}_{self.name}.csv", 'a') as f:
