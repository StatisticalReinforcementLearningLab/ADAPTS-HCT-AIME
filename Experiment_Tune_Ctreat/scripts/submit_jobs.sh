...
GLOBAL_RESULTS_DIR="Experiment_Tune_Ctreat/results"
mkdir -p "$GLOBAL_RESULTS_DIR"

# Array to store submitted job IDs
submitted_jobs=()

RUN_SCRIPT="Experiment_Tune_Ctreat/scripts/run_expt.sh"

ORDP="sbatch --mem=$MEMO -n 1 -p $PART --time=$TIME"

#Iterate through all config_general_run.json files
find Experiment_Tune_Ctreat/experiments -name "config_general_run.json" | while read config_file; do
    ...
    for alg in $algs; do
        ...
        ORD="$ORDP -J $JOBN -o $OUTF -e $ERRF $RUN_SCRIPT $alg $SCRIPT_DIR"
