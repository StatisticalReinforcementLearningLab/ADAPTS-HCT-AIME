...
RUN_SCRIPT="Experiment_Test_Opt_Policy/scripts/run_expt.sh"

ORDP="sbatch --mem=$MEMO -n 1 -p $PART --time=$TIME"

#Iterate through all config_general_run.json files
find Experiment_Test_Opt_Policy/experiments -name "config_general_run.json" | while read config_file; do
